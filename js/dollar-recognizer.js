...
class GestureTool {
    constructor() {
        ...
        this.similarity = Similarity.OptimalCos;
        this.threshold = 0.2;
        this.sampleCount = 16;
    ...
    addGesture(name, gesture) {
        if (Array.isArray(gesture) && gesture.length > 0 && Array.isArray(gesture[0])) {
            // It's an array of points, create a GestureStroke and process it
            const stroke = this.createGesture(gesture);
            stroke.resample();
            stroke.translate();
            stroke.scale();
            stroke.rotate();
        ...
        this.gesturePool.forEachGesture((name, vector, index) => {
            ...
            switch (this.similarity) {
                case Similarity.Euclidean:
                    const points = vector;
                    d = GestureUtils.squaredEuclideanDistance(points, gesture.points);
                    break;
                case Similarity.Cos:
                    d = GestureUtils.cosineDistance(vector, gesture);
                    break;
                case Similarity.OptimalCos:
                    d = GestureUtils.minimumCosineDistance(vector, gesture, this.orientationCount);
                    break;
            }
            console.log(name, d);
            if (d < minDis) {
